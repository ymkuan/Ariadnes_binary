...
class Star:
    ...
    def load_grid(self, model):
        ...
        if model.lower() == 'koesterbb':
            gridname = gridsdir + '/model_grid_KoesterBB.dat'       
        if model.lower() == 'koester':
            gridname = gridsdir + '/model_grid_Koester.dat'
        if model.lower() == 'tmap':
            gridname = gridsdir + '/model_grid_tmap.dat'
    ...
    def save_flux(self, out):
        """Save the used flux in a file."""
        master, headers = self.__prepare_flux()
        fmt = '%s %.16e %.16e'
        np.savetxt(out + 'mags2flux.dat', master, header=' '.join(headers),
                   delimiter=' ', fmt=fmt)
    ...
    def __prepare_flux(self):
        """Prepare fluxs for either printing or saving in a file."""
        flux = self.flux[np.append(self.filter_mask,
                                   self.irx_filter_mask).astype(int)]
        ers = self.flux_er[np.append(self.filter_mask,
                                      self.irx_filter_mask).astype(int)]
        filt = self.filter_names[np.append(self.filter_mask,
                                           self.irx_filter_mask).astype(int)]
        master = np.zeros(
        ...
        master['var2'] = flux
        master['var3'] = ers
        headers = ['Filter', 'Mag2Flux_ergs-1cm-2um-1', 'Uncertainty']
        return master, headers
