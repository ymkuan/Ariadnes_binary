...
class SEDPlotter:
    ...
    def __init__(self, input_files, out_folder, pdf=False,
                 model_1=None, model_2 = None, settings=None,
        ...
        if input_files != 'raw':
            ...
            self.norm_1 = out['norm1']
            self.norm_2 = out['norm2']
            if model_1 is None:
                self.grid_1 = out['model_grid1']
            else:
                self.grid_1 = model_1

            if model_2 is None:
                self.grid_2 = out['model_grid2']
            else:
                self.grid_2 = model_2

            if self.irx:
                if self.grid_1.lower() in ['phoenix', 'btsettl', 'btnextgen', 'btcond', 'kurucz', 'ck04']:
                    self.grid_1 ='btsettl' 
                if self.grid_2.lower() in ['phoenix', 'btsettl', 'btnextgen', 'btcond', 'kurucz', 'ck04']:
                    self.grid_2 ='btsettl'  
            ...
            self.star.load_grid(self.grid_1)
            self.star.load_grid(self.grid_2)
            
            if self.norm_1 and self.norm_2:
                self.order = np.array(['teff1', 'logg1', 'z1', 'teff2', 'logg2', 'z2', 'norm1', 'norm2', 'Av'])
            
            elif self.norm_1 and not self.norm_2:
                self.order = np.array(['teff1', 'logg1', 'z1', 'teff2', 'logg2', 'z2', 'dist', 'norm1', 'rad2', 'Av'])
            elif not self.norm_1 and self.norm_2:
                self.order = np.array(['teff1', 'logg1', 'z1', 'teff2', 'logg2', 'z2', 'dist', 'rad1', 'norm2', 'Av'])
            elif not self.norm_1 and not self.norm_2:
                self.order = np.array(['teff1', 'logg1', 'z1', 'teff2', 'logg2', 'z2', 'dist', 'rad1', 'rad2', 'Av'])
            ...
            self.interpolator_1 = interpolator_model(self.grid_1)
            self.interpolator_2 = interpolator_model(self.grid_2)
            ...
            model = model_grid(theta, filters, wave, self.interpolator_1, 
                               self.interpolator_2, self.norm_1, self.norm_2,
                               self.av_law, self.grid_1, self.grid_2)
            if self.irx:
                irx_model = model_grid(theta, irx_filt, irx_wave,
                                       self.interpolator_1, self.interpolator_2, 
                                       self.norm_1, self.norm_2, self.av_law, 
                                       self.grid_1, self.grid_2)

            self.theta = theta
            self.model = model
            if self.irx:
                self.irx_model = irx_model
    ...
    def plot_SED_no_model(self, title=None, s=None):
        """Plot raw photometry."""
    ...
    def SED(self, ax):
        ...
        if not self.norm_1 and not self.norm_2:
            rad1 = self.theta[7]
            rad2 = self.theta[8]
            dist = self.theta[6] * u.pc.to(u.solRad)
            norm1 = (rad1 / dist) ** 2
            norm2 = (rad2/ dist) ** 2
            Av = self.theta[9]
        elif not self.norm_1 and self.norm_2:
        ...
        # SED plot.
        grids_bi = [self.grid_1,self.grid_2]
        ww_bi = []
        flx_bi = []
        for bi, mod in enumerate(grids_bi):
            if mod == 'phoenix':
                self.star.load_grid(mod)
                ...
                brf, _ = pyasl.instrBroadGaussFast(
                    new_ww, flux, 1500, edgeHandling="firstlast",
                ...
                ww_bi.append(new_w)
                flx_bi.append(flx)
                color = getattr(self, f'model_color_{bi + 1}')
                ax.plot(new_w[:-1000], flx[:-1000], lw=1.25, color=color,
                        zorder=0, label=mod)

            elif mod == 'btsettl':
                self.star.load_grid(mod)
                wave, flux = self.fetch_btsettl(bi)
                ...
                brf, _ = pyasl.instrBroadGaussFast(
                    new_w, flux, 1500, edgeHandling="firstlast",
                    fullout=True, maxsig=8)
                ...
                flx_bi.append(flx)
                color = getattr(self, f'model_color_{bi + 1}')
                ax.plot(wave, flx, lw=1.25, color=color,
                        zorder=0, label=mod)

            elif mod == 'btnextgen':
                self.star.load_grid(mod)
                wave, flux = self.fetch_btnextgen(bi)
                ...
                self.star.load_grid(mod)
                wave, flux = self.fetch_btcond(bi)
                ...
                self.star.load_grid(mod)
                wave, flux = self.fetch_kurucz(bi)
                ...
                self.star.load_grid(mod)
                wave, flux = self.fetch_coelho(bi)
            ...
            elif mod in ['koesterbb','koester']:
                self.star.load_grid(mod)
                wave, flux = self.fetch_koester(bi)
                ...
                ext = self.av_law(wave.astype(np.float64) * 1e4, Av, Rv)
                flx = apply(ext, flux)
        ...
        for i in range(len(ww_bi)):
            data = np.column_stack((ww_bi[i], flx_bi[i]))
            np.savetxt(f'{self.out_folder}/{grids_bi[i]}_SED.dat', data, fmt=['%f','%.11e'],
                       header='wavelength(um) wave*flux(erg cm-2 s-2)', delimiter='\t')
        
        ww_add, flx_add = self.SED_binary(ax, ww_bi, flx_bi)

        min_flux = min(flx_bi[0].min(), flx_bi[1].min())
        max_flux = flx_add.max()
        ...
        if min_wave < max_wave:
            ...
            if min_wave == ww1.min():
                if interp_flx2(ww_new)[0]/interp_flx1(ww_new)[0] > 0.01:
                    ...
                    ww_new = np.concatenate((ww2[mask_below], ww_new))
                    flx_new = np.concatenate((flx2[mask_below], flx_new)) 
            
            if min_wave == ww2.min():
                if interp_flx1(ww_new)[0]/interp_flx2(ww_new)[0] > 0.01:
                    mask_below = ww1 < min_wave

                    ww_new = np.concatenate((ww1[mask_below], ww_new))
                    flx_new = np.concatenate((flx1[mask_below], flx_new)) 

            if max_wave == ww1.max():
                if interp_flx2(ww_new)[-1]/ interp_flx1(ww_new)[-1] >0.01:
                    mask_above = ww2 > max_wave

                    ww_new = np.concatenate((ww_new, ww2[mask_above]))
                    flx_new = np.concatenate((flx_new, flx2[mask_above]))
                        
            if max_wave == ww2.max():
                if interp_flx1(ww_new)[-1]/ interp_flx2(ww_new)[-1] >0.01:
                    mask_above = ww1 > max_wave

                    ww_new = np.concatenate((ww_new, ww1[mask_above]))
    ...
    def plot_corner(self, ignor = None):
        """Make corner plot."""
        ...
        if ignor is not None:
            for param in ignor:
                for i, par in enumerate(self.order):
                    if param == par:
                        self.coordinator[i] = 1
        ...
        for yi in range(theta.shape[0]):
            ...
            with open(f'{self.out_folder}/corner_data.dat', 'w') as f:
                f.write("# Parameter\tValue\tLower Bound\tUpper Bound\n")
                for param, val, lo, up in zip(used_params, theta, theta_lo, theta_up):
                    f.write(f"{param}\t{val}\t{lo}\t{up}\n")
    ...
    def fetch_Phoenix(self, bi):
        ...
        teff = self.theta[0 + dex]
        logg = self.theta[1 + dex]
        z = self.theta[2 + dex]
    ...
    def fetch_btsettl(self, bi):
        ...
        teff = self.theta[0+bi*3]
        logg = self.theta[1+bi*3]
        z = self.theta[2+bi*3]
        select_teff = np.argmin((abs(teff - np.unique(self.star.teff))))
        select_logg = np.argmin((abs(logg - np.unique(self.star.logg))))
        select_z = np.argmin((abs(z - np.unique(self.star.z))))
    ...
    def fetch_koester(self,bi):
        """Fetch correct Koester 2010 SED file.
        ...
        sel_teff = str(sel_teff) if sel_teff >= 1e4 else '0{}'.format(sel_teff)
        selected_SED = self.moddir + 'KoesterWD/koester/da' + sel_teff
        sel_logg = '{:.0f}'.format(sel_logg*100)
        selected_SED += f'_{sel_logg}.dk.dat.fits'
    ...
    def fetch_tmap(self,bi):
        """Fetch correct Subdwarf TMAP  SED file. data_4084_02/SpectralModel/Subdwarf_TMAP/sp
        ...
        sel_teff = str(sel_teff)
        selected_SED = self.moddir + 'Subdwarf_TMAP/sp/T' + sel_teff
        sel_logg = '{:.2f}'.format(sel_logg)
        selected_SED += f'_logg{sel_logg}.csv'

        hdul = pd.read_csv(selected_SED, header=None)
        flux = hdul[1].values
        wave = hdul[0].values
    ...
    def __create_titles(self, titles, theta, theta_up, theta_lo):
        ...
        for i, param in enumerate(titles):
            if param == 'teff1':
                new_titles[i] = r'Teff1 ='
            if param == 'logg1':
                new_titles[i] = r'Log g1 ='
            if param == 'z1':
                new_titles[i] = r'[Fe/H]1 ='
            if param == 'teff2':
                new_titles[i] = r'Teff2 ='
            if param == 'logg2':
                new_titles[i] = r'Log g2 ='
            if param == 'z2':
                new_titles[i] = r'[Fe/H]2 ='
            ...
            if param == 'rad1':
                new_titles[i] = r'R1 ='
            if param == 'norm1':
                new_titles[i] = r'(R1/D)$^2$ ='
            if param == 'rad2':
                new_titles[i] = r'R2 ='
            if param == 'norm2':
                new_titles[i] = r'(R2/D)$^2$ ='
            ...
            if param == 'rad1' or param == 'dist' or param == 'rad2' or param == 'Av':
                ...
                new_titles[i] += r'}$'
            elif param == 'teff1' or param == 'teff2':
                new_titles[i] += '{:.0f}'.format(theta[i])
                new_titles[i] += r'$^{+' + \
                                 '{:.0f}'.format(theta_up[i] - theta[i])
                new_titles[i] += r'}_{-' + \
                                 '{:.0f}'.format(theta[i] - theta_lo[i])
                new_titles[i] += r'}$'
    ...
    def __create_labels(self, labels):
        ...
        for i, param in enumerate(labels):
            if param == 'teff1':
                new_labels[i] = r'Teff_1 (K)'
            if param == 'logg1':
                new_labels[i] = r'Log g_1'
            if param == 'z1':
                new_labels[i] = r'[Fe/H]_1'
            if param == 'teff2':
                new_labels[i] = r'Teff_2 (K)'
            if param == 'logg2':
                new_labels[i] = r'Log g_2'
            if param == 'z2':
                new_labels[i] = r'[Fe/H]_2'
            
            if param == 'dist':
                new_labels[i] = r'D (pc)'
            if param == 'rad1':
                new_labels[i] = r'R_1 $($R$_\odot)$'
            if param == 'norm1':
                new_labels[i] = r'(R_1/D)'
            if param == 'rad2':
                new_labels[i] = r'R_2 $($R$_\odot)$'
            if param == 'norm2':
                new_labels[i] = r'(R_2/D)'
            if param == 'Av':
                new_labels[i] = r'Av'
            if param == 'inflation':
