...
class Fitter:
    ...
    @norm_1.setter
    def norm_1(self, norm_1):
        if type(norm_1) is not bool:
    ...
    @norm_2.setter
    def norm_2(self, norm_2):
        if type(norm_2) is not bool:
            InputError('norm must be True or False.').__raise__()
        self._norm_2 = norm_2
    ...
    def grid(self, grid):
        ...
        if grid.lower() == 'coelho':
            with open(gridsdir + '/Coelho_DF.pkl', 'rb') as intp:
                self._interpolator = DFInterpolator(pd.read_pickle(intp))
        if grid.lower() == 'koesterbb':
            with open(gridsdir + '/KoesterBB_DF.pkl', 'rb') as intp:
                self._interpolator = DFInterpolator(pd.read_pickle(intp))
        if grid.lower() == 'koester':
            with open(gridsdir + '/Koester_DF.pkl', 'rb') as intp:
                self._interpolator = DFInterpolator(pd.read_pickle(intp))
        if grid.lower() == 'tmap':
            with open(gridsdir + '/Tmap_DF.pkl', 'rb') as intp:
                self._interpolator = DFInterpolator(pd.read_pickle(intp))
    ...
    def initialize(self):
        ...
        up global variables.
        """
        global prior_dict, coordinator, fixed, order, star, use_norm_1, use_norm_2, av_law
        ...
        use_norm_1 = self.norm_1
        use_norm_2 = self.norm_2

        # Extinction law
        av_law = self._av_law

        # Declare order of parameters.
        if not self.norm_1 and not self.norm_2:
            order = np.array(
                [
            ...
            npars = 10
        elif not self.norm_1 and self.norm_2:
            order = np.array(
                [
                    'teff1', 'logg1', 'z1', 'teff2', 'logg2', 'z2',
                    'dist', 'rad1', 'norm2', 'Av'
            ...
            npars = 10
        elif self.norm_1 and not self.norm_2:
            order = np.array(
                [
                    'teff1', 'logg1', 'z1', 'teff2', 'logg2', 'z2',
                    'dist', 'norm1', 'rad2', 'Av'
            ...
            npars = 10
        elif self.norm_1 and self.norm_2:
        ...
        self.star.save_flux(self.out_folder + '/')
    ...
    def get_ndim(self):
        ...
        ndim = 10 if not self.norm_1 or not self.norm_2 else 9
    ...
    def _default_priors(self):
        ...
        if self.star.get_logg:
            ...
            defaults['logg1'] = st.uniform(loc=3.5, scale=2.5)
            defaults['logg2'] = st.uniform(loc=3.5, scale=2.5)
            for i, mod in enumerate(self._models) :
                if mod.lower() in ['koester', 'koesterbb']:
                    defaults[f'logg{i+1}'] = st.uniform(loc=6.5, scale=3)
                if mod.lower() == 'tmap':
                    defaults[f'logg{i+1}'] = st.uniform(loc=4.5, scale=2)

        # Teff prior from RAVE
        with open(priorsdir + '/teff_ppf.pkl', 'rb') as jar:
            defaults['teff1'] = pickle.load(jar)
        defaults['teff2'] = defaults['teff1']
        for i, mod in enumerate(self._models) :
            if mod.lower() in ['koester', 'koesterbb']:
                defaults[f'teff{i+1}'] = st.uniform(loc=5000, scale=75000)
            if mod.lower() == 'tmap':
                defaults[f'teff{i+1}'] = st.uniform(loc=20000, scale=80000)

            
        # [Fe/H] prior setup.
        defaults['z1'] = st.norm(loc=-0.125, scale=0.234)
        defaults['z2'] = st.norm(loc=-0.125, scale=0.234)
        for i, mod in enumerate(self._models) :
            if mod.lower() in ['koester', 'koesterbb', 'tmap']:
                z_idx = i + (i + 1) * 2
                value = 0
                self.coordinator[z_idx] = 1
                self.fixed[z_idx] = value
                defaults[f'z{i+1}'] = st.uniform(loc=0, scale=0)

        # Distance prior setup.
        if not self._norm_1 or not self._norm_2:
            if self.star.dist != -1:
                defaults['dist'] = st.truncnorm(
                    a=0, b=1e100, loc=self.star.dist, scale=3 * self.star.dist_e
                )
            else:
                defaults['dist'] = st.uniform(loc=1, scale=4000)
            # Radius prior setup.
        #if not self._norm_1:
        defaults['rad1'] = st.uniform(loc=0.05, scale=100)
        #else:
        defaults['norm1'] = st.uniform(loc=0, scale=1e-20) 
        #if not self._norm_2:
        defaults['rad2'] = st.uniform(loc=0.05, scale=100)
        #else:
        defaults['norm2'] = st.uniform(loc=0, scale=1e-20)     

        for i, mod in enumerate(self._models) :
            if mod.lower() in ['koester', 'koesterbb']:
                if i == 0 and not self._norm_1:
                   # defaults['rad1'] = st.uniform(loc=0.002, scale = 0.05-0.002)
                   defaults['rad1'] = st.uniform(loc=0.0005, scale = 0.5-0.0005)
                if i == 1 and not self._norm_2:
                    # defaults['rad2'] = st.uniform(loc=0.002, scale = 0.05-0.002)
                    defaults['rad2'] = st.uniform(loc=0.0005, scale = 0.5-0.0005)
            if mod.lower() == 'tmap':
                if i == 0 and not self._norm_1:
        ...
        if self.star.Av == 0.:
            av_idx = 8 if self._norm_1 and self.norm_2 else 9
    ...
    def create_priors_from_setup(self):
        ...
        if ('norm1'in keys and 'rad1'in keys) or ('norm2'in keys and 'rad2'in keys) or ('norm1'in keys and 'norm2'in keys and 'dist' in keys):
            er = PriorError('rad1 or dist1 or rad2 or norm12', 1)
    ...
    def fit_binary(self):
        """Perform the fit with different models.
        ...
        global interpolator_1, interpolator_2,model_1, model_2
        interpolator_1 = self._interpolators[0]
        interpolator_2 = self._interpolators[1]

        self.grid_1 = self._grids[0]
        self.grid_2 = self._grids[1]
        model_1 = self.grid_1
        model_2 = self.grid_2

        grids_name = f'{self.grid_1}_{self.grid_2}'
        out_file = self.out_folder + '/' + grids_name + '_out.pkl'
        print('\t\t\tFITTING MODELS : ' + grids_name) 
       
        try:
            self.fit_dynesty(out_file=out_file)
        
        except ValueError as e:
            dump_out = self.out_folder + '/' + grids_name + '_DUMP.pkl'
            pickle.dump(self.sampler.results, open(dump_out, 'wb'))
            DynestyError(dump_out, grids_name, e).__raise__()
            # continue
        ...
        elapsed_time = execution_time(self.start)
            ...
            grids_name, self.norm_1, self.norm_2)
        pass
    ...
    def save(self, out_file, results=None):
        ...
        if use_norm_1 and star.dist != -1:
            rad1 = self._get_rad(
                out['posterior_samples']['norm1'], star.dist, star.dist_e
            )
            out['posterior_samples']['rad1'] = rad1
        
        if use_norm_2 and star.dist != -1:
            rad2 = self._get_rad(
                out['posterior_samples']['norm2'], star.dist, star.dist_e
            )
            out['posterior_samples']['rad2'] = rad2
        # Create a distribution of masses.

        logg1_samp = out['posterior_samples']['logg1']
        logg2_samp = out['posterior_samples']['logg2']
        rad1_samp = out['posterior_samples']['rad1']
        rad2_samp = out['posterior_samples']['rad2']

        mass1_samp = self._get_mass(logg1_samp, rad1_samp)
        mass2_samp = self._get_mass(logg2_samp, rad2_samp)
        out['posterior_samples']['grav_mass1'] = mass1_samp
        out['posterior_samples']['grav_mass2'] = mass2_samp

        # Create a distribution of luminosities.

        teff1_samp = out['posterior_samples']['teff1']
        teff2_samp = out['posterior_samples']['teff2']
        lum1_samp = self._get_lum(teff1_samp, rad1_samp)
        lum2_samp = self._get_lum(teff2_samp, rad2_samp)
        out['posterior_samples']['lum1'] = lum1_samp
        out['posterior_samples']['lum2'] = lum2_samp

        # Create a distribution of angular diameters.

        if not use_norm_1 or not use_norm_2:
            dist_samp = out['posterior_samples']['dist']
            ad1_samp = self._get_angular_diameter(rad1_samp, dist_samp)
            out['posterior_samples']['AD1'] = ad1_samp
            ad2_samp = self._get_angular_diameter(rad2_samp, dist_samp)
            out['posterior_samples']['AD2'] = ad2_samp

        for i in range(posterior_samples.shape[0]):
            theta = build_params(
                posterior_samples[i, :], flux, flux_er, filts, coordinator,
                fixed, self.norm_1, self.norm_2)
            out['posterior_samples']['loglike'][i] = log_likelihood(
                theta, flux, flux_er, wave, filts, self._interpolators[0], self._interpolators[1], self.norm_1,
                self.norm_2, av_law, self.grid_1, self.grid_2)
        ...
        for i, param in enumerate(order):
            if not self.coordinator[i]:
                ...
                if param == 'z1':
                    logdat = out_filler(samp, logdat, param, '[Fe/H]_1', out)
                elif param == 'z2':
                    logdat = out_filler(samp, logdat, param, '[Fe/H]_2', out) 
                elif param == 'norm1':
                    logdat = out_filler(samp, logdat, param, '(R1/D)^2',
                                        out, fmt='e')
                    if star.dist != 1:
                        logdat = out_filler(
                            out['posterior_samples']['rad1'], logdat, 'rad1',
                            'R1', out
                        )
                elif param == 'norm2':
                    logdat = out_filler(samp, logdat, param, '(R2/D)^2',
                                        out, fmt='e')
                    if star.dist != 1:
                        logdat = out_filler(
                            out['posterior_samples']['rad2'], logdat, 'rad2',
                            'R2', out
                        )
                else:
                ...
                if param == 'z1':
                    logdat = out_filler(0, logdat, param, '[Fe/H]_1', out, fixed=self.fixed[i])
                elif param == 'z2':
                    logdat = out_filler(0, logdat, param, '[Fe/H]_2', out, fixed=self.fixed[i])  
                else:
                    logdat = out_filler(0, logdat, param, param, out, fixed=self.fixed[i])

            best_theta[i] = out['best_fit'][param]

        # Add derived mass to best fit dictionary.

        samp = out['posterior_samples']['grav_mass1']
        logdat = out_filler(
            samp, logdat, 'grav_mass1', 'grav_mass1', out)
        
        samp = out['posterior_samples']['grav_mass2']
        logdat = out_filler(
            samp, logdat, 'grav_mass2', 'grav_mass2', out)
        # Add derived luminosity to best fit dictionary.

        samp = out['posterior_samples']['lum1']
        logdat = out_filler(samp, logdat, 'lum1', 'lum1', out)
        samp = out['posterior_samples']['lum2']
        logdat = out_filler(samp, logdat, 'lum2', 'lum2', out)
        
        # Add derived angular diameter to best fit dictionary.

        if not use_norm_1:
            samp = out['posterior_samples']['AD1']
            logdat = out_filler(samp, logdat, 'AD1', 'AD1', out)
        if not use_norm_2:
            samp = out['posterior_samples']['AD2']
            logdat = out_filler(samp, logdat, 'AD2', 'AD2', out)
        ...
        out['best_fit']['loglike'] = log_likelihood(
            ...
            filts, self._interpolators[0], self._interpolators[1], self.norm_1, self.norm_2, 
            av_law, self.grid_1, self.grid_2)
        ...
        # Find spt
        spt1_idx = np.argmin(abs(mamajek_temp - out['best_fit']['teff1']))
        spt2_idx = np.argmin(abs(mamajek_temp - out['best_fit']['teff2']))
        spt1 = mamajek_spt[spt1_idx]
        spt2 = mamajek_spt[spt2_idx]
        out['spectral_type_1'] = spt1
        out['spectral_type_2'] = spt2
        logdat += f'Mamajek Spectral Type for star1\t{spt1}\n'
        logdat += f'Mamajek Spectral Type for star2\t{spt2}\n'
        ...
        out['norm1'] = self.norm_1
        out['norm2'] = self.norm_2
        out['model_grid1'] = self.grid_1
        out['model_grid2'] = self.grid_2
        out['av_law'] = av_law
        #if not self.bma:
    ...
    def save_bma(self, avgd):
        ...
        #age_samp, mass_samp, eep_samp = [0,0,0]
       #age_samp, mass_samp, eep_samp = self.estimate_age(
        ...
        for i, param in enumerate(order):
            if not self.coordinator[i]:
                ...
                logdat_average = out_filler(sampw, logdat_samples, param, param,
    ...
    def load_interpolator(model):
        ...
        if model.lower() == 'phoenix':
            ...
            with open(gridsdir + '/Coelho_DF.pkl', 'rb') as intp:
                df = DFInterpolator(pd.read_pickle(intp))
        elif model.lower() == 'koester':
            with open(gridsdir + '/Koester_DF.pkl', 'rb') as intp:
                df = DFInterpolator(pd.read_pickle(intp))
        elif model.lower() == 'koesterbb':
            with open(gridsdir + '/KoesterBB_DF.pkl', 'rb') as intp:
                df = DFInterpolator(pd.read_pickle(intp))
        elif model.lower() == 'tmap':
            with open(gridsdir + '/Tmap_DF.pkl', 'rb') as intp:
                df = DFInterpolator(pd.read_pickle(intp))
        return df
...
def dynesty_log_like(cube):
    ...
    theta = build_params(
        cube, flux, flux_er, filts, coordinator, fixed, use_norm_1, use_norm_2
    )
    return log_likelihood(theta, flux, flux_er, wave,filts, interpolator_1, 
...
def pt_dynesty(cube):
    ...
    return prior_transform_dynesty(cube, flux, flux_er, filts, prior_dict,
                                   coordinator, use_norm_1, use_norm_2)
