...
def end(coordinator, elapsed_time, out_folder, engine, use_norm_1, use_norm_2):
    ...
    if not use_norm_1:
        ad1 = out['best_fit']['AD1']
        unlo, unhi = out['uncertainties']['AD1']
        lo, up = out['confidence_interval']['AD1']
        fmt_str += f'\t\t\tAngular Diameter for star1: {ad1:.4f} '
        fmt_str += f'+ {unhi:.4f} - {unlo:.4f} [{lo:.4f}, {up:.4f}]\n'
    if not use_norm_2:
        ad2 = out['best_fit']['AD2']
        unlo, unhi = out['uncertainties']['AD2']
        lo, up = out['confidence_interval']['AD2']
        fmt_str += f'\t\t\tAngular Diameter for star2: {ad2:.4f} '
        fmt_str += f'+ {unhi:.4f} - {unlo:.4f} [{lo:.4f}, {up:.4f}]\n'

    mass1 = out['best_fit']['grav_mass1']
    unlo, unhi = out['uncertainties']['grav_mass1']
    lo, up = out['confidence_interval']['grav_mass1']
    fmt_str += f'\t\t\tGrav mass for star1 : {mass1:.4f} '
    fmt_str += f'+ {unhi:.4f} - {unlo:.4f} [{lo:.4f}, {up:.4f}]\n'

    mass2 = out['best_fit']['grav_mass2']
    unlo, unhi = out['uncertainties']['grav_mass2']
    lo, up = out['confidence_interval']['grav_mass2']
    fmt_str += f'\t\t\tGrav mass for star2 : {mass2:.4f} '
    fmt_str += f'+ {unhi:.4f} - {unlo:.4f} [{lo:.4f}, {up:.4f}]\n'

    lum1 = out['best_fit']['lum1']
    unlo, unhi = out['uncertainties']['lum1']
    lo, up = out['confidence_interval']['lum1']
    fmt_str += f'\t\t\tStar1 Luminosity : {lum1:.4f} '
    fmt_str += f'+ {unhi:.4f} - {unlo:.4f} [{lo:.4f}, {up:.4f}]\n'

    lum2 = out['best_fit']['lum2']
    unlo, unhi = out['uncertainties']['lum2']
    lo, up = out['confidence_interval']['lum2']
    fmt_str += f'\t\t\tStar2 Luminosity : {lum2:.4f} '
    fmt_str += f'+ {unhi:.4f} - {unlo:.4f} [{lo:.4f}, {up:.4f}]\n'
    ...
    spt1 = out['spectral_type_1']
    spt2 = out['spectral_type_2']
    print(colored('\t\t\tMamajek Spectral Type for star1 : ', c), end='')
    print(colored(spt1, c))
    print(colored('\t\t\tMamajek Spectral Type for star2 : ', c), end='')
    print(colored(spt2, c))
...
def out_filler(samp, logdat, param, name, out, fmt='f', fixed=False,
    ...
    if fixed is False:
        ...
        out[f'best_fit'][param] = best

        logdat += f'{name}\t{best:.4{fmt}}\t'
        out[f'uncertainties'][param] = (best - lo, up - best)
        logdat += f'{up - best:.4{fmt}}\t{best - lo:.4{fmt}}\t'

        out[f'confidence_interval'][param] = (lo3, up3)
        logdat += f'{lo:.4{fmt}}\t{up:.4{fmt}}\n'
    else:
        out[f'best_fit'][param] = fixed
        out[f'uncertainties'][param] = np.nan
        out[f'confidence_interval'][param] = np.nan
