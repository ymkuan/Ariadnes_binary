...
class SEDPlotter:
    ...
    def __init__(self, input_files, out_folder, pdf=False,
        ...
        if input_files != 'raw':
            ...
            model = model_grid(theta, filters, wave,
    ...
    def SED(self, ax):
        ...
        if self.grid == 'phoenix':
            ...
            flux = brf * norm * new_w
            wave = new_w
            ax.plot(wave[:-1000], flux[:-1000], lw=1.25, color=self.model_color,
                    zorder=0)
        ...
        elif self.grid in ['koesterbb','koester']:
            wave, flux = self.fetch_koester()
        ...
        np.savetxt(f'{self.out_folder}/{self.grid}_spectrum.dat', data, fmt=['%f','%.11e'], 
                   header='wavelength(mu m) wave*flux(erg cm-2 s-2)', delimiter = '\t')
        pass
