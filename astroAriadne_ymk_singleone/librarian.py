...
class Librarian:
    ...
    def gaia_params(self):
        ...
        if len(res) > 1:
            ...
            non_missing_counts = res_df.notna().sum(axis=1)
            max_non_missing_index = non_missing_counts.idxmax()
            res_df_best = res_df.loc[[max_non_missing_index]]
            res = Table.from_pandas(res_df_best)
