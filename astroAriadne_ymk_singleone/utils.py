...
__all__ = ['estimate_pdf', 'estimate_cdf', 'norm_fit', 'credibility_interval',
           'display_routine', 'display_star_fin', 'display_star_init', 'end','endone',
           'get_noise_name', 'create_dir', 'execution_time', 'out_filler','out_filler_one']
...
def endone(coordinator, elapsed_time, out_folder, engine, use_norm):
    """Display end of run information.
...
def out_filler_one(samp, logdat, param, name, out, fmt='f', fixed=False,
               method='averaged'):
    """Fill up the output file."""
    if method not in ['averaged', 'samples']:
        raise Exception('Method is wrong!')
