...
from .config import (filesdir, gridsdir, priorsdir, filter_names, colors,
                     iso_mask, iso_bands, filter_names_spec)
from .error import *
...
class Fitter:
    ...
    def _default_priors(self):
        ...
        if self.star.get_logg:
            ...
            for i, mod in enumerate(self._bma_models):
                # 从先验设置上，感觉白矮星模板和其他的一起搞，还是不太好
                if mod.lower() in ['koester', 'koesterbb']:
                    defaults['logg'] = st.uniform(loc=6.5, scale =3)
                if mod.lower() == 'tmap':
                    defaults['logg'] = st.uniform(loc=4.5, scale=2)
        ...
        for i, mod in enumerate(self._bma_models) :
            if mod.lower() in ['koester', 'koesterbb']:
                defaults['teff'] = st.uniform(loc=5000, scale=75000)
            if mod.lower() == 'tmap':
                defaults['teff'] = st.uniform(loc=20000, scale=80000)
        ...
        for i, mod in enumerate(self._bma_models) :
            if mod.lower() in ['koester', 'koesterbb', 'tmap']:
                ...
                self.fixed[2] = value
                defaults['z'] = st.uniform(loc=0, scale=0)
        ...
        if not self._norm:
            ...
            for i, mod in enumerate(self._bma_models):
                if mod.lower() in ['koester', 'koesterbb']:
                    defaults['rad']=st.uniform(loc=0.004, scale = 0.04-0.004)
                if mod.lower() == 'tmap':
                    defaults['rad']=st.uniform(loc=0.04, scale = 0.55-0.04) 
    ...
    def fit(self):
        ...
        pass
    def fit_one(self):
        """Perform the fit with one model and not do the average.

        Only works with dynesty.
        """  
        # Assuming this is inside a loop (for example, iterating over star models or filters)
        for model in self._bma_models:
            if len(self.star.filter_names[self.star.filter_mask]) <= 5:
                if model in ['koester', 'koesterbb'] and len(self.star.filter_names[self.star.filter_mask]) >= 2:
                    continue  # Skip to the next iteration if the condition is met，有的时候要注意去掉这部分
                else:
        ...
        # Now that the fitting finished, read the outputs 
        # the posteriors
        outs = []
        ...
        elapsed_time = execution_time(self.start)
        endone(self.coordinator, elapsed_time, self.out_folder,
            self._bma_models[0], self.norm)
        pass
        ...
        if not self.bma:
            ...
            for i, param in enumerate(order):
                if not self.coordinator[i]:
                    ...
                    if param == 'z':
                        logdat = out_filler_one(samp, logdat, param, '[Fe/H]', out)
                    elif param == 'norm':
                        logdat = out_filler(samp, logdat, param, '(R/D)^2',
                                            out, fmt='e')
                        if star.dist != 1:
                            logdat = out_filler_one(
                                out['posterior_samples']['rad'], logdat, 'rad',
                                'R', out
                            )
                    else:
                        logdat = out_filler_one(samp, logdat, param, param, out)
                else:
                    logdat = out_filler_one(
            ...
            if self.bma:
                ...
                logdat = out_filler_one(samp, logdat, 'grav_mass', 'grav_mass', out)
            ...
            if self.bma:
                ...
                logdat = out_filler_one(samp, logdat, 'lum', 'lum', out)
            ...
            if not use_norm:
                ...
                if self.bma:
                    ...
                    logdat = out_filler_one(samp, logdat, 'AD', 'AD', out)
            ...
            for i, param in enumerate(order):
                if not self.coordinator[i]:
                    ...
                    if self.bma:
                        ...
                        logdat = out_filler_one(samp, logdat, param, param, out, fmt='f')
    ...
    def save_bma(self, avgd):
       ...
       #age_samp, mass_samp, eep_samp = self.estimate_age(
        ...
        if max_prob_mod.lower() in ['tmap', 'koester', 'koesterbb']:
            fluxes = np.zeros((len(oglogg), len(filter_names_spec)))     
            for i, t, g in zip(range(len(oglogg)), ogteff, oglogg):
                intp = self.load_interpolator(max_prob_mod.lower()) 
                values = (g, t, 0)
                fluxes[i,:] = intp(values, filter_names_spec)
        else:
        ...
        if max_prob_mod.lower() in ['tmap', 'koester', 'koesterbb']:
            for i, filt in enumerate(filter_names_spec):
                samp = fluxes[:, i]
        ...
        spt = mamajek_spt[spt_idx]
        out['spectral_type'] = spt
        logdat_average +=  f'Mamajek Spectral Type for star\t{spt}\n'
